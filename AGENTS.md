...
- 文案引擎默认走**云雾中转**（yunwu.ai，OpenAI 兼容，一把 key 通 400+ 模型），默认模型 `deepseek-v4-flash`。模型工厂 `lib/model.ts` 的 `getModel()` 统一取模型；`LLM_PROVIDER=deepseek` 可一键切回 DeepSeek 官方直连（备用，需 `DEEPSEEK_API_KEY`）。云雾走 `createOpenAICompatible` 必开 `supportsStructuredOutputs`，否则 `generateObject` 返回裸文本解析为空。云雾 key 配置见桌面《配置信息/云雾API-中转.md》
...
`LLM_PROVIDER`（yunwu 默认 / deepseek）`YUNWU_API_KEY` `LLM_MODEL`（默认 deepseek-v4-flash）`DEEPSEEK_API_KEY`（切回官方直连时用）`RESEND_API_KEY` `MAIL_TO` `CRON_SECRET` `BLOB_READ_WRITE_TOKEN`（Blob store 连接项目后自动注入，本地要用 `vercel env pull`）`SITE_URL`
