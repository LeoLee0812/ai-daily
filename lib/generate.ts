...
// 第二段：逐条抓原文全文，DeepSeek 深挖成完整条目（正文/表格/怎么玩/点评）
// 收尾：汇总生成今日要点与小结
...
选题标准（按优先级）：模型/重要产品发布或重大升级 > 个人开发者马上能上手的新工具 > 行业重大事件（收购/融资/监管）。同一主题只选一条，优先一手来源。
...
const itemSchema = z.object({
  ...
  comment: z
    .string()
    .describe("Leo 点评草稿：一两句第一人称判断（会不会用/值不值得跟），口语化"),
});
